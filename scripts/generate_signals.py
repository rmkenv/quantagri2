...
Reads NDVI flat files, calls Ollama Cloud (qwen2.5), writes signals.
Output: data/signals/latest.json + data/signals/{YYYY-MM-DD}.json
"""
...
from config import NDVI_DIR, SIG_DIR, NODES
...
{{
  ...
  "featureBlocks": [
    {{"name": "NDVI Velocity",      "status": "Pass"|"Caution"|"Fail", "signal": "", "weight": 0.25}},
    {{"name": "LSWI Moisture",      "status": "Pass"|"Caution"|"Fail", "signal": "", "weight": 0.20}},
    {{"name": "Cloud Cover QA",     "status": "Pass"|"Caution"|"Fail", "signal": "", "weight": 0.10}},
    {{"name": "Seasonal Alignment", "status": "Pass"|"Caution"|"Fail", "signal": "", "weight": 0.20}},
    {{"name": "Macro Regime",       "status": "Pass"|"Caution"|"Fail", "signal": "", "weight": 0.25}}
  ]
}}"""


def load_latest_ndvi(commodity: str, region: str) -> dict | None:
    files = sorted(NDVI_DIR.glob(f"{commodity}_{region}_*.json"))
    if not files:
        return None
...
def run():
    ...
    print(f"\n[SIGNALS] {date_str} — {len(NODES)} nodes\n")
    ...
    print(f"\n[SIGNALS] {len(results)} signals written")
    if errors:
        print(f"[SIGNALS] {len(errors)} skipped: {errors}")
